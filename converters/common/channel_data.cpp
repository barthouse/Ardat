...
{
	sprintf_s(in_buffer, in_buffer_length, "acceleration %lfg, %lfg",
		m_lateral_acceleration, m_longitudinal_acceleration);
}
...
{
	sprintf_s(in_buffer, in_buffer_length, "time stamp %.2fs", m_time_stamp);
}
...
{
	sprintf_s(in_buffer, in_buffer_length, "gps position (%lf,%lf) accuracy %lfmm",
...
{
	sprintf_s(in_buffer, in_buffer_length, "gps speed %lfm/s accuracy %lfm/s", m_gps_speed, m_gps_speed_accuracy);
}
...
{
	sprintf_s(in_buffer, in_buffer_length, "gps altitude %lfmm", m_z);
}
...
void c_gps_altitude::set_data(unsigned char * in_data, int in_data_length)
{
    ...
    unsigned int z_data = (m_data[1] << 24) | (m_data[2] << 16) | (m_data[3] << 8) | m_data[4];

	if ((z_data & 0x80000000) == 0x80000000)
	{
        m_z = (double) ((z_data ^ 0xffffffff) + 1);
        m_z = -m_z;
	}
	else
	{
